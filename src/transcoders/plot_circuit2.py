...
def visualize_full_circuit(full_circuit):
    # Create a new directed graph
    G = nx.DiGraph()

    # Add nodes to the graph
    for node, data in full_circuit["nodes"].items():
        G.add_node(node, **data)

    # Add edges to the graph
    for edge in full_circuit["edges"]:
    ...
    node_sizes = [data['attribution'] * 1000 for node, data in G.nodes(data=True) if 'attribution' in data]
    
    # Calculate edge widths based on attribution
    edge_widths = [data['attribution'] * 5 for (u, v, data) in G.edges(data=True) if 'attribution' in data]

    # Create custom positions for nodes with jitter
    pos = {}
    for node in G.nodes():
        x = extract_horizontal_position(node.reduction) + random.uniform(-0.3, 0.3)
        y = extract_vertical_position(node.hook_point) + random.uniform(-0.6, 0.6)
        pos[node] = (x, y)
    ...
    # Add a title
    plt.title("Full Circuit Visualization (Sorted with Jitter)")
