...
def apply_and_run(
    ...
    def sae_bwd_hook(output_grads: torch.Tensor, hook:HookPoint):
        print(hook.name)
        return (torch.where(output_grads * 1   > threshold,output_grads,0),)

    def tc_bwd_hook(output_grads: torch.Tensor, hook:HookPoint):
        print(hook.name)
        return (torch.where(output_grads * 1   > threshold,output_grads,0),)
    ...
    for hook_point,tc in tcs_dict.items():
        ...
        bwd_hooks.append((hook_point,tc_bwd_hook))
